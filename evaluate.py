...
def evaluate_model(model, dataloader, criterion, device, threshold=0.6):
    """
    Evaluates CTHBNet using global pixel-level segmentation metrics.

    Metrics are accumulated across the complete dataset before
    calculating IoU, Dice, Precision, and Recall.
    """
    model.eval()
    ...
    with torch.no_grad():
        for images, masks, fids in dataloader:
            ...
            # Same global metric logic used in analyze_thresholds.py
            probabilities = torch.sigmoid(extent_logits)
            predictions = probabilities > threshold
            targets = masks.unsqueeze(1) > 0.5
